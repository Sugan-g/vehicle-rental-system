...
export default function BookingPage() {
    ...
    const fetchData = async () => {
        try {
            ...
            reviewRes.data.forEach(r => {
                // r.vehicle might be populated object or a string id
                const vehicleId = r.vehicle && r.vehicle._id ? r.vehicle._id : r.vehicle;
    ...
    const handleSubmitReview = async (b) => {
        ...
        try {
            ...
            // If backend returns the created review object
            // pick vehicle id safely (res.data.vehicle could be id or object)
            const returnedVehicleId = res.data.vehicle && res.data.vehicle._id
                ? res.data.vehicle._id
                : res.data.vehicle || b.vehicle._id;
            ...
            // clear input for that booking
            setReviewInputs(prev => ({
                ...prev,
                [b._id]: { rating: 0, comment: "" }
            }));

            // Optionally re-fetch to ensure backend/populated data is in sync
            await fetchData();
                ...
                >
                    ...
                    {b.status !== "cancelled" && (
                        <div className="mt-5 border-t pt-3">
                            {reviews[b.vehicle._id] && reviews[b.vehicle._id].rating > 0 ? (
