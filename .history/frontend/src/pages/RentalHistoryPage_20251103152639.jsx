...
export default function RentalHistoryPage({ booking }) {
  ...
  useEffect(() => {
    // 🧩 Prevent crash when booking or vehicle not yet loaded
    if (!booking || !booking.vehicle) return;

    const fetchReview = async () => {
      try {
        const res = await API.get("/reviews/my");
        const found = res.data.find(
          (r) => r.vehicle?._id === booking.vehicle._id
        );
        if (found) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!booking || !booking.vehicle) return; // safeguard
    try {
      setLoading(true);
  ...
  // ✅ Defensive render to avoid "undefined" crash
