...
export default function BookingPage() {
  ...
  the [reviews, setReviews] = useState({});
  ...
  const handlePayNow = async (booking) => {
    try {
      ...
      if (!amount || !booking._id) {
        ...
        console.error("Missing data:", { amount, bookingId: booking._id });
        return;
      }
  ...
  // NEW — CANCEL PAYMENT FLOW (frontend only)
      ...
      {bookings.map((b) => {
        ...
        return (
          <div key={b._id} className="border p-4 rounded-xl mb-5 shadow bg-white">
            ...
            {/* Edit + Cancel + Pay Now + Cancel Payment */}
            {b.status !== "cancelled" && (
