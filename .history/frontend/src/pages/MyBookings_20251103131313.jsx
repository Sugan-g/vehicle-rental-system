...
export default function BookingPage() {
    ...
    const fetchData = async () => {
        try {
            ...
            reviewRes.data.forEach(r => {
                const vehicleId = r.vehicle?._id || r.vehicle; // handle object or string id
                if (vehicleId) {
                    mapped[vehicleId] = {
                        rating: r.rating,
                        comment: r.comment,
    ...
    const handleSubmitReview = async (b) => {
        ...
        try {
            ...
            const vehicleId = res.data.vehicle?._id || res.data.vehicle || b.vehicle._id;
            ...
            // Clear textarea + stars
            setReviewInputs(prev => ({
                ...prev,
                [b._id]: { rating: 0, comment: "" },
            }));

            // Optional — fetch again to ensure backend sync
            await fetchData();
